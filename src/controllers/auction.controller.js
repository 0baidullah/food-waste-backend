import User from "../models/user.model.js"; // 👈 Capitalized singular name
...
const accountSid = 'AC905d14200aeaf7c90f84b9b72b847071';
const authToken = '2a267f62f565fe871a5f80df47553b54';
const twilioPhone = '+16012927421';
const client = twilio(accountSid, authToken);



function formatPhoneNumber(phone) {
  phone = phone.replace(/[\s-]/g, ""); // Remove dashes/spaces

  if (phone.startsWith("+")) return phone;  // If already in international format, return as is
  if (phone.startsWith("0")) return "+92" + phone.slice(1);  // If starts with "0", add +92
  if (phone.length === 10) return "+92" + phone;  // If the phone number length is 10 digits, prepend +92
  return phone;  // Return the phone number as is if it doesn't match expected formats
}
...
const createAuction = asyncHandler(async (req, res) => {
  try {
    ...
    if (!name || !description || !category || !startingPrice || !location || !image) {
    ...
    // If checkbox is checked, ensure startTime and endTime are provided and valid
    let status = "active"; // Default status when there is no start and end time
    if (checkbox === 'true') {
      if (!startTime || !endTime) {
        return res.status(400).json(new ApiResponse(400, "Start time and end time are required when checkbox is selected"));
      }
      if (new Date(startTime).getTime() >= new Date(endTime).getTime()) {
        return res.status(400).json(new ApiResponse(400, "Start time must be before end time"));
      }

      // Determine status based on startTime, endTime, and current time
    ...
    const auction = await Auction.create({
      ...
      category,
      seller: req.user._id,
      startTime: checkbox === 'true' ? startTime : null,
      endTime: checkbox === 'true' ? endTime : null,
      location,
    ...
    // Send SMS notification to  all users phone  that this auction is created
    ...
    //   client.messages
    //     .create({
    //       body: `New auction created: ${name}. Check it out!`,
    //       from: twilioPhone,
    //       to: user.phone,
    ...
    users.forEach((users) => {
      if (users.phone) {
        const formattedPhone = formatPhoneNumber(users.phone); // Format phone number to include +92
 console.log(formattedPhone, "formatted phone number");
        // Send SMS
        client.messages
          .create({
            body: `New auction created: ${name}. Check it out!`,
...
const getAllAuctions = asyncHandler(async (req, res) => {
  try {
    ...
    .populate("seller", "fullName address city location")
    .populate({
      path: "winner",
    ...
    //show new ones
    .sort({ createdAt: -1 });
    console.log(auctions[0].seller.location); 
